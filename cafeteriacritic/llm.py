...
class OllamaLLM(LLM):
    ...
    def send_prompt(self, prompt: str) -> str:
        ...
        response = requests.post(f'http://localhost:{self._port}/api/generate', json=data)
...
class OpenAILLM(LLM):
    ...
    _SYSTEM_PROMPT = ('You are a food critic assistant, skilled in evaluating '
    ...
    def __init__(self, model: str = 'gpt-3.5-turbo', api_key: str = None) -> None:
        ...
        if len(api_key) != 51:
            raise ValueError((f'Invalid API key: length is incorrect '
                              f'expected: 51; actual: {len(api_key)}'))
        if api_key[0:3] != 'sk-':
            raise ValueError((f'Invalid API key: first 3 characters are incorrect '
...
def menu_collection_to_prompt(menu_collection: MenuCollection) -> str:
    ...
    Returns:
        A string prompting a LLM to make a decision about which menu is the
        most appealing.
    """
    prompt = ("Below is a list of restaurants/cafeterias and their menus "
