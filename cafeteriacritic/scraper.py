...
class CafeBonAppetitScraper(ABC):
    ...
    def get_menu(building: str, meal: str = 'lunch', day: date = None) -> Menu:
        ...
        if building not in CafeBonAppetitScraper._CAFETERIAS.keys():
            raise ValueError((f'Invalid building: {building}; valid options are: '
                              f'{", ".join(CafeBonAppetitScraper._CAFETERIAS.keys())}'))
        if meal not in CafeBonAppetitScraper._MEALS:
            raise ValueError((f'Invalid meal {meal}; valid options are: '
        ...
        menu = CafeBonAppetitScraper._menu_soup_to_menu(page_soup,
                                                        building=building,
                                                        meal=meal)
        return menu
    ...
    def _fetch_menu_html(building: str, day: date) -> requests.Response:
        url = CafeBonAppetitScraper._URL_BASE.format(
            CafeBonAppetitScraper._CAFETERIAS[building],
            day.strftime('%Y-%m-%d')
        )
        response = requests.get(url)
        if response.status_code != 200:
            raise RuntimeError(f'Error fetching page; HTTP response code {response.status_code}')
        return response
    ...
    def _menu_soup_to_menu(page_soup: BeautifulSoup, building: str, meal: str) -> Menu:
        ...
        for item_soup in meal_soup.find_all(class_='site-panel__daypart-item'):
            name = item_soup.find(class_='h4 site-panel__daypart-item-title').text.strip()
            price = float('NaN')
            ...
            items.append(MenuItem(name=name, price=price, desc=desc, tags=frozenset(tags)))
